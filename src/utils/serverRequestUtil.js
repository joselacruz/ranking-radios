...
export async function makeServerRequest({ endpoint, offset=0,limit,otherQuery=null }) {
...
for (const server of serverList) {
  ...
  try {
   ...
   const laurl = `${serverUrl}${endpoint}?offset=${offset}&limit=${limit}${queryString ? `&${queryString}` : ''}`;
   
   const response = await fetch(`${serverUrl}${endpoint}?offset=${offset}&limit=${limit}${queryString ? `&${queryString}` : ''}`, {
